...
async function minifyDir(dir) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      const code = fs.readFileSync(full, 'utf-8');
      const result = await Terser.minify(code, { compress: true, mangle: { reserved: ['addToCart', 'loadProducts', 'renderProducts', 'initProductPage', 'toggleTheme', 'submitSuggestion', 'switchTab'] } });
